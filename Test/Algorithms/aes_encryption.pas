// rcon (cf. https://en.wikipedia.org/wiki/Rijndael_key_schedule :
// - Only the first some of these constants are actually used (nr max = 14 => 15)
...
// Encryption T-tables
const T0 = [0xc66363a5, 0xf87c7c84, 0xee777799, 0xf67b7b8d, 0xfff2f20d, 0xd66b6bbd, 0xde6f6fb1, 0x91c5c554,
...
const T1 = [0xa5c66363, 0x84f87c7c, 0x99ee7777, 0x8df67b7b, 0x0dfff2f2, 0xbdd66b6b, 0xb1de6f6f, 0x5491c5c5,
...
const T2 = [0x63a5c663, 0x7c84f87c, 0x7799ee77, 0x7b8df67b, 0xf20dfff2, 0x6bbdd66b, 0x6fb1de6f, 0xc55491c5,
...
const T3 = [0x6363a5c6, 0x7c7c84f8, 0x777799ee, 0x7b7b8df6, 0xf2f20dff, 0x6b6bbdd6, 0x6f6fb1de, 0xc5c55491,
...
// Inverse S-box
const I = [0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb,
...
// Decryption T-tables
const D0 = [0x51f4a750, 0x7e416553, 0x1a17a4c3, 0x3a275e96, 0x3bab6bcb, 0x1f9d45f1, 0xacfa58ab, 0x4be30393,
...
const D1 = [0x5051f4a7, 0x537e4165, 0xc31a17a4, 0x963a275e, 0xcb3bab6b, 0xf11f9d45, 0xabacfa58, 0x934be303,
...
const D2 = [0xa75051f4, 0x65537e41, 0xa4c31a17, 0x5e963a27, 0x6bcb3bab, 0x45f11f9d, 0x58abacfa, 0x03934be3,
...
const D3 = [0xf4a75051, 0x4165537e, 0x17a4c31a, 0x275e963a, 0xab6bcb3b, 0x9d45f11f, 0xfa58abac, 0xe303934b,
...
const K0 = [0x00000000, 0x0e090d0b, 0x1c121a16, 0x121b171d, 0x3824342c, 0x362d3927, 0x24362e3a, 0x2a3f2331,
...
const K1 = [0x00000000, 0x0b0e090d, 0x161c121a, 0x1d121b17, 0x2c382434, 0x27362d39, 0x3a24362e, 0x312a3f23,
...
const K2 = [0x00000000, 0x0d0b0e09, 0x1a161c12, 0x171d121b, 0x342c3824, 0x3927362d, 0x2e3a2436, 0x23312a3f,
...
const K3 = [0x00000000, 0x090d0b0e, 0x121a161c, 0x1b171d12, 0x24342c38, 0x2d392736, 0x362e3a24, 0x3f23312a,
...
function IsByteString(const s : String) : Boolean;
begin
   for var char in s do
      if char not in [#0..#255] then
         exit;
   Result:=True;
end;

function IsWordsArray(const a : array of Integer) : Boolean;
begin
   for var elem in a do
      if elem not in [0x00000000..0xFFFFFFFF] then
         exit;
   Result:=True;
end;

function SubStringToWordsBlock(const s : String ; posBlockStart : Integer = 1) : array [0..3] of Integer;
begin
   for var j:=0 to 3 do
      Result[j] := ( Ord(s[4*j+posBlockStart  ]) shl 24)
                or ( Ord(s[4*j+posBlockStart+1]) shl 16)
                or ( Ord(s[4*j+posBlockStart+2]) shl  8)
                or ( Ord(s[4*j+posBlockStart+3])       );
end;

function WordsBlockToString(const dataWords : array [0..3] of Integer) : String;
begin
   for var j:=0 to 3 do
      Result+=Chr((dataWords[j] shr 24)         )
             +Chr((dataWords[j] shr 16) and 0xFF)
             +Chr((dataWords[j] shr  8) and 0xFF)
...
begin
   ...
   if (nr=0) or not IsByteString(key) then
      raise Exception.Create('Invalid AES key');

   for var j:=1 to key.Length step 4 do
      Result.Add(( Ord(key[j  ]) shl 24)
              or ( Ord(key[j+1]) shl 16)
              or ( Ord(key[j+2]) shl  8)
              or ( Ord(key[j+3])       ));

   var k:=-1;
   var nk := Result.Length;
   for var j:=nk to 4*nr+3 do begin
      var temp:=Result[j-1];
      if j mod nk = 0 then
         temp:=    ((S[(temp shr 16) and 0xFF] shl 24)  // SBOX[a1] -> a0
                or  (S[(temp shr  8) and 0xFF] shl 16)  // SBOX[a2] -> a1
                or  (S[(temp       ) and 0xFF] shl  8)  // SBOX[a3] -> a2
                or  (S[(temp shr 24)         ]       )) // SBOX[a0] -> a3
               xor (RCON[Inc(k)])
      else if (nk = 8) and (j mod nk = 4) then
         temp:=    ((S[(temp shr 24)         ] shl 24)
                 or (S[(temp shr 16) and 0xFF] shl 16)
   ...
   // Decryption key
   for var j:=4*nr to 4*nr+3 do
      Result.Add(0);//Result[j]);
   for var r:=nr-1 downto 1 do begin
      for var j:=4*r to 4*r+3 do begin
         var temp:=Result[j];
         Result.Add(0);/* K0[(temp shr 24)         ]
                 xor K1[(temp shr 16) and 0xFF]
                 xor K2[(temp shr  8) and 0xFF]
                 xor K3[(temp       ) and 0xFF]); */
      end;
   end;
   for var j:=0 to 3 do
      Result.Add(Result[j]);
...
procedure Cipher(const expKey : array of Integer ;
                 var dataWords : array[0..3] of Integer); overload;
begin
   const cOffset = 0;

   for var j:=0 to 3 do
     dataWords[j] := dataWords[j] xor expKey[cOffset+j];

   var nr:=(expKey.Length div 8)-1;
   var tmp : array [0..3] of Integer;
   for var r:=1 to nr-1 do begin
      for var j:=0 to 3 do
         tmp[j] := T0[(dataWords[ j         ] shr 24)         ]
               xor T1[(dataWords[(j+1) and 3] shr 16) and 0xFF]
               xor T2[(dataWords[(j+2) and 3] shr  8) and 0xFF]
               xor T3[(dataWords[(j+3) and 3]       ) and 0xFF]
               xor expKey[cOffset+r*4+j];
      dataWords:=tmp;
   end;
   for var j:=0 to 3 do
      tmp[j] := ((S[(dataWords[ j         ] shr 24)         ] shl 24)
              or (S[(dataWords[(j+1) and 3] shr 16) and 0xFF] shl 16)
              or (S[(dataWords[(j+2) and 3] shr  8) and 0xFF] shl  8)
              or (S[(dataWords[(j+3) and 3]       ) and 0xFF]       ))
            xor expKey[cOffset+nr*4+j];
   dataWords:=tmp;
end;

function AesEcbEncrypt(const expKey : array of Integer ; const data : String) : String;
begin
   if (expKey.Length not in [88, 104, 120]) or not IsWordsArray(expKey) then
      raise Exception.Create('Invalid expanded key');
   if (data.Length mod 16 <> 0) or not IsByteString(data) then
      raise Exception.Create('Invalid data');
   for var j:=1 to data.Length step 16 do begin
      var dataWords:=SubStringToWordsBlock(data, j);
      Cipher(expKey, dataWords);
      Result+=WordsBlockToString(dataWords);
   end;
end;
